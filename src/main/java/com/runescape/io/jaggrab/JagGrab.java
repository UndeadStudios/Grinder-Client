...
public class JagGrab extends JagGrabConstants {
    ...
    public static void requestCrcs() {
        while (!crcsLoaded() && !Client.instance.exitRequested) {
            ...
            try (DataInputStream in = openJagGrabRequest("crc")) {
                ...
                LoginScreen.drawLoadingText(Client.instance, 20, "Requesting CRCs..");
    ...
    private static void requestPreload(int i, String fileName) {
        ...
        while (buffer == null && !Client.instance.exitRequested) {
            LoginScreen.drawLoadingText(Client.instance, 20, "Requesting " + fileName);
    ...
    public static void error(String error) {
        ...
        for (int remaining = delay; remaining > 0; remaining--) {
            if (failedAttempts >= 10) {
                LoginScreen.drawLoadingText(Client.instance, 10, "Game updated - please reload page");
                remaining = 10;
            } else {
                LoginScreen.drawLoadingText(Client.instance, 10, "" + error + " Error - Retrying in " + remaining + " seconds.");
            }
            try {
                Thread.sleep(1000L);
     ...
     *
     * @param in The socket to read bytes from.
     */
    ...
    public static DataInputStream openJagGrabRequest(String filePath) throws IOException {
        ...
        if (socket != null) {
            try {
                ...
                Log.error("Failed to open jag grab socket", e);
            }
            socket = null;
        }

        //Open new jaggrab request
        socket = Client.instance.createSocket(FILE_SERVER_PORT);
