...
public final class AreaDefinition {
    ...
    public static AreaDefinition lookup(int area) {
        ...
        if (area >= 0 && area <= size) {
            area_data.index = indices[area];
            data.readValues(area_data);

            if (!sprites.containsKey(data.spriteId)) {
                try {
                    sprites.put(data.spriteId, new Sprite(Client.instance.mediaArchive, "mapfunction", data.spriteId));
                } catch (Exception e) {
                    System.out.println("Missing Sprite: " + data.spriteId + " Using Shop Icon");
                    sprites.put(data.spriteId, new Sprite(Client.instance.mediaArchive, "mapfunction", 0));
                }
            }
        }
        return data;
